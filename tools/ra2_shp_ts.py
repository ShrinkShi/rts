...
# x, y, width, height, flags, frame RGBA, reserved, data offset
FRAME_HEADER = struct.Struct("<4HI4BII")
FLAG_HAS_TRANSPARENCY = 0x01
FLAG_USES_RLE = 0x02
...
class ShpTsFile:
    ...
    def from_bytes(cls, payload: bytes, source_name: str = "") -> "ShpTsFile":
        ...
        for index in range(frame_count):
            ...
            frame_color = (red << 24) | (green << 16) | (blue << 8) | alpha
            if width and height:
                ...
                if data_offset < table_end or data_offset >= len(payload):
                    raise ShpTsError(
                        f"SHP frame {index} data offset is invalid: {data_offset}: "
                        f"{source_name}"
                    )
            headers.append((x, y, width, height, flags, frame_color, data_offset))

        offsets = sorted({item[-1] for item in headers if item[-1] >= table_end})
        next_offset = {
            value: offsets[index + 1] if index + 1 < len(offsets) else len(payload)
            for index, value in enumerate(offsets)
        }

        frames: list[ShpFrame] = []
        for index, (x, y, width, height, flags, frame_color, data_offset) in enumerate(
            headers
        ):
            if width == 0 or height == 0:
                ...
                frame_payload = payload[
                    data_offset : next_offset.get(data_offset, len(payload))
                ]
...
def _decode_scanline_rle(
    ...
    for row in range(height):
        if cursor + 2 > len(payload):
            raise ShpTsError(
                f"SHP frame {frame_index} RLE line {row} header is truncated: "
                f"{source_name}"
            )
        ...
        while cursor < line_end and x < width:
            ...
            if cursor >= line_end:
                raise ShpTsError(
                    f"SHP frame {frame_index} RLE line {row} ends after a skip "
                    f"marker: {source_name}"
                )
            skip = payload[cursor]
            cursor += 1
            if skip == 0:
                break
            # Real Westwood files can encode a transparent run past the remaining
            # row width. The game reader clamps the run to the row boundary.
            x = min(width, x + skip)
        cursor = line_end
    return bytes(output)
...
def indexed_to_rgba(
    ...
    if len(indices) != width * height:
        raise ShpTsError(
            f"Indexed image length mismatch: expected {width * height}, "
            f"got {len(indices)}"
        )
    if len(palette) != 256:
