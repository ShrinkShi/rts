...
func _draw() -> void:
    ...
    var body_color := Color("#50565C") if powered else Color("#3A3D40")
    var light_color := Color("#8F989E") if powered else Color("#5A5E61")
    if damaged:
    ...
    draw_polyline(PackedVector2Array([housing[0], housing[1], housing[2], housing[3], housing[0]]), Color("#1C1F22"), 1.2)

    draw_line(direction * 2.0, direction * 18.0, Color("#171A1C"), 5.5)
    draw_line(direction * 3.0, direction * 18.0, light_color, 2.2)
    draw_line(direction * 6.0 + side * 2.5, direction * 15.0 + side * 2.5, Color("#32373B"), 1.5)
    draw_line(direction * 6.0 - side * 2.5, direction * 15.0 - side * 2.5, Color("#32373B"), 1.5)
