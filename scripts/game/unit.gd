...
@export_enum("rifle", "rocket", "tank", "scout", "harvester")
var preset_unit_id: String = "rifle"
...
var visual_sprite: AnimatedSprite2D
var turret_sprite: AnimatedSprite2D
var damage_smoke_anchor: Marker2D
var cargo_bar_anchor: Marker2D
var selection_anchor: Marker2D
...
func _build_visual():
	...
	visual_sprite = get_node_or_null("VisualRoot/Body") as AnimatedSprite2D
	turret_sprite = get_node_or_null("VisualRoot/Turret") as AnimatedSprite2D
	damage_smoke_anchor = get_node_or_null("DamageSmokeAnchor") as Marker2D
	cargo_bar_anchor = get_node_or_null("CargoBarAnchor") as Marker2D
	selection_anchor = get_node_or_null("SelectionAnchor") as Marker2D
	prefab_visual_nodes = is_instance_valid(visual_root) and is_instance_valid(visual_sprite)

	if not ra2_entity_id.is_empty():
		var manifest: Dictionary = RA2RuntimeDatabase.load_manifest(ra2_entity_id)
		var use_layered_vehicle: bool = (
			str(stats.get("category", "infantry")) == "vehicle"
			and manifest.has("layered_vehicle")
		)
		var runtime_width: float = float(ra2_profile.get(
			"target_width",
			25.0 if str(stats.get("category", "infantry")) == "infantry" else 54.0
		))
		var runtime_ground_y: float = float(ra2_profile.get("ground_y", 2.0))
		...
		# Keep the two concrete visual classes in separate branches. A ternary expression
		# makes Godot infer the shared API from RA2VisualPlayer, whose play_state() accepts
		# only three arguments, and then rejects the layered vehicle's four-argument call.
		if use_layered_vehicle:
			var layered_visual = RA2LayeredVehicleVisual.new()
			layered_visual.name = "RA2LayeredVehicle"
			add_child(layered_visual)
			if layered_visual.setup(
				ra2_entity_id,
				team_color,
				str(ra2_profile.get("theater", "temperate"))
			):
				ra2_visual = layered_visual
				ra2_layered_visual = true
				if is_instance_valid(visual_root):
					visual_root.visible = false
				layered_visual.configure_layout(runtime_width, runtime_ground_y, runtime_offset)
				layered_visual.play_state(
					"stand", visual_direction, turret_visual_direction, true
				)
				visual_state = "stand"
				return
			layered_visual.queue_free()
		else:
			var flat_visual = RA2VisualPlayer.new()
			flat_visual.name = "RA2Visual"
			add_child(flat_visual)
			if flat_visual.setup(
				ra2_entity_id,
				team_color,
				str(ra2_profile.get("theater", "temperate"))
			):
				ra2_visual = flat_visual
				ra2_layered_visual = false
				if is_instance_valid(visual_root):
					visual_root.visible = false
				flat_visual.configure_layout(runtime_width, runtime_ground_y, runtime_offset)
				flat_visual.play_state("stand", visual_direction, true)
				visual_state = "stand"
				return
			flat_visual.queue_free()
	...
	if not prefab_visual_nodes:
		...
		visual_sprite.position = base_visual_position
		visual_sprite.scale = Vector2.ONE * target_scale
		if unit_id == "tank":
			turret_sprite = AnimatedSprite2D.new()
			turret_sprite.name = "Turret"
			turret_sprite.position = base_visual_position + (visual_profile.turret_offset if visual_profile != null else Vector2.ZERO)
			turret_sprite.scale = Vector2.ONE * target_scale * (visual_profile.turret_scale_multiplier if visual_profile != null else Vector2.ONE)
			visual_root.add_child(turret_sprite)

	if visual_sprite.sprite_frames == null or not visual_sprite.sprite_frames.has_animation("stand_0"):
	...
	if unit_id == "tank" and is_instance_valid(turret_sprite):
...
func _has_independent_turret() -> bool:
	return ra2_layered_visual or unit_id == "tank"
...
func _play_visual_animation(state_name, restart = false):
	if is_instance_valid(ra2_visual):
		if ra2_layered_visual:
			ra2_visual.play_state(
				str(state_name), visual_direction, turret_visual_direction, bool(restart)
			)
		else:
			ra2_visual.play_state(str(state_name), visual_direction, bool(restart))
		visual_state = str(state_name)
		return
...
func _update_visual_state(delta):
	if not is_instance_valid(ra2_visual) and not is_instance_valid(visual_sprite):
		return
	...
	if dying:
		...
		return
	if deploying:
		_play_visual_animation("deploy")
		return
	if deployed:
		var deployed_state: String = "deployed_attack" if attack_animation_time > 0.0 else "deployed"
		if is_instance_valid(attack_target):
			facing_direction = global_position.direction_to(attack_target.global_position)
		var deployed_direction: int = _direction_index(facing_direction)
		var deployed_changed: bool = deployed_direction != visual_direction
		visual_direction = deployed_direction
		if deployed_state != visual_state or deployed_changed:
			_play_visual_animation(deployed_state, deployed_changed)
		return
...
func _update_damage_smoke():
	...
	if should_smoke and not is_instance_valid(damage_smoke):
		...
		damage_smoke.position = damage_smoke_anchor.position if is_instance_valid(damage_smoke_anchor) else Vector2(0, -25)
...
func _process_death(delta):
	...
	if death_elapsed > corpse_lifetime - 1.2:
		var death_alpha: float = clamp((corpse_lifetime - death_elapsed) / 1.2, 0.0, 1.0)
		if is_instance_valid(ra2_visual):
			ra2_visual.set_alpha(death_alpha)
		elif is_instance_valid(visual_sprite):
			visual_sprite.modulate.a = death_alpha
			if is_instance_valid(turret_sprite):
				turret_sprite.modulate.a = death_alpha
	queue_redraw()
...
func _build_collision_shape():
	...
	max_slides = 8
	collision_shape_node = get_node_or_null("CollisionShape2D") as CollisionShape2D
	...
	var circle = collision_shape_node.shape as CircleShape2D
	if circle == null:
...
func _physics_process(delta):
	...
	if deploying:
		deploy_transition_time = maxf(0.0, deploy_transition_time - delta)
		velocity = Vector2.ZERO
		if deploy_transition_time <= 0.0:
			deploying = false
			deployed = true
			visual_state = ""
		_update_visual_state(delta)
	...
	if deployed:
		velocity = Vector2.ZERO
		if is_instance_valid(attack_target):
			_process_attack_target(delta, false, false)
		elif scan_cooldown <= 0.0:
			scan_cooldown = 0.25
			_acquire_guard_target(global_position, _effective_guard_range())
		_update_visual_state(delta)
...
func _weapon_range() -> float:
	var base_value: float = float(stats.get("range", 0.0))
	return base_value * 1.38 if deployed and ra2_entity_id == "E1" else base_value

func _weapon_reload() -> float:
	var base_value: float = float(stats.get("reload", 1.0))
	return base_value * 0.58 if deployed and ra2_entity_id == "E1" else base_value

func _weapon_damage() -> float:
	var base_value: float = float(stats.get("damage", 0.0))
	return base_value * 1.15 if deployed and ra2_entity_id == "E1" else base_value

func _effective_guard_range():
	var weapon_range = _weapon_range()
...
func _process_hold(delta):
	...
	if not auto_attack_enabled or not is_combat_unit():
		...
		return
	if is_instance_valid(attack_target):
		if global_position.distance_to(attack_target.global_position) <= _weapon_range():
			_process_attack_target(delta, false, true)
		else:
			attack_target = null
	elif scan_cooldown <= 0.0:
		scan_cooldown = 0.25
		_acquire_guard_target(hold_position, _weapon_range())
...
func _process_attack_target(delta, allow_chase = true, obey_chase_limit = true):
	...
	if distance <= range_value:
		var moving_attack: bool = (
			_has_independent_turret()
			and str(active_order.get("type", "")) in ["attack_move", "patrol"]
...
func _fire_at(target):
	...
	var damage = _weapon_damage() * veterancy_bonus
	if str(stats.get("projectile_type", "")) == "shell":
		match_ref.spawn_projectile(
			self, target, target.global_position, damage,
			float(stats.get("aoe_radius", 0.0)),
			float(stats.get("projectile_speed", 330.0)),
			float(stats.get("projectile_arc", 44.0))
		)
	else:
...
func _fire_at_position(target_position, forced_target = null):
	...
	if str(stats.get("projectile_type", "")) == "shell":
		match_ref.spawn_projectile(
			self, forced_target, target_position, damage,
			float(stats.get("aoe_radius", 0.0)),
			float(stats.get("projectile_speed", 330.0)),
			float(stats.get("projectile_arc", 44.0))
		)
	...
	for other in nearby_units:
		...
		if global_position.distance_to(other.global_position) <= own_radius * 0.72 + other_radius * 0.72:
			...
			if owner_id == 0 and crush_voice_cooldown <= 0.0:
				...
				if not ra2_entity_id.is_empty():
					if not RA2RuntimeDatabase.play_entity_role(ra2_entity_id, "VoiceSpecialAttack", 100):
						RA2RuntimeDatabase.play_entity_role(ra2_entity_id, "VoiceAttack", 100)
				else:
					VoiceManager.speak(unit_id, "crush")

func can_deploy() -> bool:
	return ra2_entity_id == "E1" and not dying

func command_deploy() -> bool:
	if not can_deploy():
		return false
	order_queue.clear()
	active_order = {}
	...
	path = PackedVector2Array()
	velocity = Vector2.ZERO
	if deployed or deploying:
		deployed = false
		deploying = false
		deploy_transition_time = 0.0
		visual_state = ""
		_play_visual_animation("stand", true)
		if owner_id == 0:
			EventBus.notification_requested.emit("美国大兵已解除部署", "info")
	else:
		deploying = true
		deployed = false
		deploy_transition_time = 0.72
		visual_state = ""
		_play_visual_animation("deploy", true)
		if owner_id == 0:
			EventBus.notification_requested.emit("美国大兵正在部署重机枪阵地", "info")
	return true

func _undeploy_for_movement() -> void:
	if deployed or deploying:
		deployed = false
		deploying = false
		deploy_transition_time = 0.0
...
func command_move(target_position, manual = true, queued = false):
	_undeploy_for_movement()
	if bool(manual) and owner_id == 0 and not ra2_entity_id.is_empty():
		RA2RuntimeDatabase.play_entity_role(ra2_entity_id, "VoiceMove")
...
func command_attack(target, queued = false):
	...
	if target.has_method("is_resource_entity") and target.is_resource_entity():
		return
	if owner_id == 0 and not ra2_entity_id.is_empty():
		RA2RuntimeDatabase.play_entity_role(ra2_entity_id, "VoiceAttack")
...
func enter_refinery(refinery):
	...
	if refinery.has_method("set_special_active"):
		refinery.set_special_active(true)
	velocity = Vector2.ZERO
	visible = false
	collision_layer = 0
	collision_mask = 0
...
func exit_refinery(success, exit_position):
	...
	if is_instance_valid(unload_refinery) and unload_refinery.has_method("set_special_active"):
		unload_refinery.set_special_active(false)
...
func set_selected(value):
	...
	if selected and not was_selected and owner_id == 0 and not ra2_entity_id.is_empty():
		RA2RuntimeDatabase.play_entity_role(ra2_entity_id, "VoiceSelect")
	queue_redraw()
...
func _draw():
	...
	if selected:
		var selection_center = selection_anchor.position if is_instance_valid(selection_anchor) else Vector2(0, 7)
		draw_set_transform(selection_center, 0.0, Vector2(1.0, 0.42))
	...
	if not dying and selected and unit_id == "harvester":
		...
		var cargo_y = cargo_bar_anchor.position.y if is_instance_valid(cargo_bar_anchor) else radius + 19.0
