...
var spread_interval: float = 10.0
...
func _process(delta: float) -> void:
    ...
    if spread_elapsed >= spread_interval:
        spread_elapsed = 0.0
        spread_interval = 8.5 + float(posmod(
            int(Time.get_ticks_msec()) + cell.x * 7 + cell.y * 11,
            8
        )) * 0.7
        spread_requested.emit(cell)
...
func _draw() -> void:
    var pillar_texture: Texture2D = RA2OriginalTextures.texture(
        "tibtre01_%02d" % animation_frame
    )
    if pillar_texture == null:
        draw_circle(Vector2(0, -8), 12.0, Color("#D0A52A"))
        return
    var texture_size: Vector2 = pillar_texture.get_size()
    var target_width: float = 54.0
    var scale_value: float = target_width / maxf(1.0, texture_size.x)
