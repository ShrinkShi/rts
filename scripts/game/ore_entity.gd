...
func _refresh_overlay_texture() -> void:
    ...
    if not is_instance_valid(map_ref) or get_remaining() <= 0:
        return
    var ratio := clampf(float(get_remaining()) / float(maxi(1, max_remaining)), 0.0, 1.0)
    var stage := clampi(int(round(ratio * 11.0)), 0, 11)
...
func get_selection_rect() -> Rect2:
    var size_value := Vector2(42.0, 34.0)
...
func _draw() -> void:
    ...
    var ratio := clampf(float(remaining) / float(maxi(1, max_remaining)), 0.0, 1.0)
    if remaining > 0:
...
func _draw_overlay_texture(ratio: float) -> void:
    ...
    if texture_size.x <= 0.0 or texture_size.y <= 0.0:
        return
    var scale_value := (map_ref.tile_px * 1.55 / texture_size.x) * lerpf(0.86, 1.0, ratio)
    var size_value := texture_size * scale_value
    var target_rect := Rect2(
        Vector2(-size_value.x * 0.5, map_ref.tile_px * 0.48 - size_value.y),
        size_value
    )
...
func _draw_fallback(ratio: float) -> void:
    var radius := lerpf(7.0, 15.0, ratio)
    draw_circle(Vector2(0, 3), radius, Color("#C89D24"))
    draw_circle(Vector2(-4, -2), radius * 0.55, Color("#F2D55B"))
