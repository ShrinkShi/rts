...
static func panel_style(bg: Color = Color("#18212A"), border: Color = Color("#4D5D6B"), radius: int = 4) -> StyleBoxFlat:
...
static func button_style(bg: Color, border: Color, radius: int = 3) -> StyleBoxFlat:
    var style: StyleBoxFlat = StyleBoxFlat.new()
    style.bg_color = bg
    style.border_color = border
    style.set_border_width_all(1)
...
static func style_button(button: Button, primary: bool = false) -> void:
    var normal_bg: Color = Color("#31566B") if primary else Color("#202D38")
    var hover_bg: Color = Color("#3C6A82") if primary else Color("#2B3C49")
...
static func add_labeled_control(parent: Container, caption: String, control: Control) -> HBoxContainer:
...
static func style_compact_button(button: Button, primary: bool = false, minimum_width: float = 132.0) -> void:
    var normal_bg: Color = Color("#31566B") if primary else Color("#202D38")
    var hover_bg: Color = Color("#3C6A82") if primary else Color("#2B3C49")
    for state_name in ["normal", "hover", "pressed", "focus", "disabled"]:
        var bg: Color = normal_bg
        var border: Color = Color("#607D8B")
        if state_name == "hover" or state_name == "focus":
            ...
            bg = Color("#11191E")
            border = Color("#31434B")
        var style: StyleBoxFlat = button_style(bg, border, 3)
...
static func style_compact_option(option: OptionButton, minimum_width: float = 82.0) -> void:
    for state_name in ["normal", "hover", "pressed", "focus", "disabled"]:
        var bg: Color = Color("#131C24")
        var border: Color = Color("#526572")
        if state_name == "hover" or state_name == "focus":
            bg = Color("#1D2B35")
            border = Color("#7893A3")
        elif state_name == "pressed":
...
static func color_swatch_texture(hex_value: Variant, swatch_size: Vector2i = Vector2i(28, 18)) -> ImageTexture:
    var color_text: String = str(hex_value)
    if not color_text.begins_with("#"):
        color_text = "#" + color_text
    var image: Image = Image.create(swatch_size.x, swatch_size.y, false, Image.FORMAT_RGBA8)
    image.fill(Color("#0A1014"))
    var fill_color: Color = Color(color_text)
    for y in range(2, swatch_size.y - 2):
        for x in range(2, swatch_size.x - 2):
            image.set_pixel(x, y, fill_color)
    return ImageTexture.create_from_image(image)

static func add_color_swatch_items(option: OptionButton, color_definitions: Array) -> void:
    for item in color_definitions:
        var label_text: String = str(item[0])
        var hex_value: String = str(item[1])
